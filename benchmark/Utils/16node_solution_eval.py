...
# STEP 3: not minority and jailed
...
for run_id in run_ids:
    ...
    GDI_67 = numpy.percentile(all_data['two_third_dist'], 67)
...
summary = results_df.groupby('minority_count').mean()
summary.to_csv('/home/ubuntu/results/summary_minority_jailed_16node.csv', index=True)
print(summary)

# summarized_df = pd.DataFrame(columns=['minority_count', 'num_jailed_5','num_jailed_10', 'num_jailed_20']) 
# for minority_count in minority_count_array:
#     print(minority_count)
#     data = results_df[results_df['minority_count'] == minority_count]
    
#     new_data = pd.DataFrame({'minority_count' : minority_count, 'num_jailed_5' : }, index=[0]) 
#     print(new_data)
