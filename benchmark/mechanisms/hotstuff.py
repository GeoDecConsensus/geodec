...
class HotStuffLogParser:
    def __init__(self, clients, nodes, faults):
        ...
        proposals, commits, sizes, self.received_samples, timeouts, self.configs = zip(*results)
    ...
    def _parse_nodes(self, log):
        ...
        configs = {
            "consensus": {
                ...
                "sync_retry_delay": int(search(r"consensus.* Sync retry delay .* (\d+)", log).group(1)),
            },
            "mempool": {
                "gc_depth": int(search(r"Garbage collection .* (\d+)", log).group(1)),
                "sync_retry_delay": int(search(r"mempool.* Sync retry delay .* (\d+)", log).group(1)),
...
class HotStuffMechanism:
    def __init__(self, settings):
        ...
        self.update_cmd = [
            ...
            CommandMaker.alias_binaries(f"./{self.settings.repo_name}/target/release/", self.settings.repo_name),
        ]
